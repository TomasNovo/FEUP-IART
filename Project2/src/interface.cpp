...
void info()
{
	...
	cout << "Game devoloped based on play store game Labyrinth Characters." << endl << endl;
...
Node* play_loop(Node* rootNode)
{
	...
	while (!currNode->finished())
	{
		...
		{
			cout << endl << "Character numbers: " << endl;
			...
			for (int i = 0; i < currNode->state.size(); i++)
			{
				...
				cout << "Character " << tmp <<  " number -> " << i <<  endl;
			}

			cout << "\nSelect character: ";
			cin >> characterIndex;

			while(characterIndex >= characters.size() || characterIndex < 0)
			{
...
int play(string map)
{
	...
	cout << "- Input the letter of the character that you want to move (A-Z)" << endl;
	cout << "- Input u,d,l,r to go up, down, left and right, respectively " << endl;
 	cout << "- By pressing one of these keys, your character will move in that direction " << "and it will only stop if an obstacle is in its way" << endl;
...
void flashingAnimation(Node* node)
{
	...
	vector<Character> stateSave = node->state;
	...
	for (int i = 0; i < map.size(); ++i)
	{
		for (int j = 0; j < map[i].size(); ++j)
		{
			...
			if (node != NULL)
			{
				...
				{
					if (i == node->state[k].coords[1] && j == node->state[k].coords[0]) // character's position
					{
						cout << (char)(node->state[k].id + ('A' - 'a'));
						break;
					}

					else if (i == node->state[k].objective[1] && j == node->state[k].objective[0]) // character's position
					{
